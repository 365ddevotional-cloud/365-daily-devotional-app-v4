import { db } from "./db";
import {
  getFallbackDevotionals,
  getFallbackDevotionalByDate,
  getFallbackLatestDevotional,
} from "./devotional-fallback";
import {
  devotionals,
  prayerRequests,
  prayerReplies,
...
export class DatabaseStorage implements IStorage {
  ...
  async getDevotionals(): Promise<Devotional[]> {
    try {
      ...
      console.error("[storage] Failed to load devotionals from database, using fallback dataset", error);
      return await getFallbackDevotionals();
    }
  }
  ...
  async getDevotional(id: number): Promise<Devotional | undefined> {
    try {
      ...
      console.error(`[storage] Failed to load devotional ${id} from database, using fallback dataset`, error);
  ...
  async getDevotionalByDate(date: string): Promise<Devotional | undefined> {
    try {
      ...
      console.error(`[storage] Failed to load devotional for ${date} from database, using fallback dataset`, error);
      return await getFallbackDevotionalByDate(date);
    }
  }
  ...
  async getLatestDevotional(): Promise<Devotional | undefined> {
    try {
      ...
      console.error("[storage] Failed to load latest devotional from database, using fallback dataset", error);
      return await getFallbackLatestDevotional();
    }
  }
